    ...
    public class GridColumn<T, TDataType> : GridColumnBase<T> where T : class
    {
        ...
        public override IGridCell GetValue(T instance)
        {
            ...
            if (IsCheck)
            {
                if (_checkBoxId != null)
                {
                    textValue = "<input ID=" + _checkBoxId(instance) + " class=\"check-item\" id=\"Selected\" name=\"Selected\" type=\"checkbox\" value=\"true\" /><input name=\"Selected\" type=\"hidden\" value=\"false\" />";
                }
            }
            else
            {
