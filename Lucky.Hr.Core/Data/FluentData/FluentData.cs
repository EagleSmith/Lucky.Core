...
namespace Lucky.Hr.Core
{
	...
	public partial interface  IDbCommand : IExecute, IExecuteReturnLastId, IQuery, IParameterValue, IDisposable
	{
	...
	internal class AutoMapper
	{
		...
		{
            //EntityBuilder<Object> _eb = EntityBuilder<Object>.CreateBuilder((Lucky.Hr.Core.DataReader)_reader);
            //while (_reader.Read())
            //{
            //    item = _eb.Build((Lucky.Hr.Core.DataReader)_reader);
            //}
		...
		private object GetOrCreateInstance(object item, PropertyInfo property)
		{
			...
			if (instance == null)
			{
				...
				property.FastSetValue(item, instance);
			}
