...
namespace Lucky.Core.Plugins
{
    public class PluginManager
    {
        ...
        /// <summary>
        /// 返回所有引用的插件组件的集合，它已经被阴影复制
        /// </summary>
        public static IEnumerable<PluginDescriptor> ReferencedPlugins { get; set; }

        /// <summary>
        /// 返回所有插件的集合而不是与当前版本不兼容
        /// </summary>
        ...
        {
            using (new WriteLockDisposable<DirectoryInfo>().Read())
            {
                // 可以在这里添加详细的异常处理，因为这是发生在应用程序启动之前
                ...
                try
                {
                    ...
                    Debug.WriteLine("创建卷影副本的文件夹，并查找插件dll");
                    //确保创建文件夹
                    ...
                    if (_clearShadowDirectoryOnStartup)
                    {
                        ...
                        foreach (var f in binFiles)
                        {
                            ...
                            {
                                Debug.WriteLine("删除文件 " + f.Name + "错误。错误: " + exc);
                            }
                        }
                    }
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        //确保在该版本的插件是有效的
                        if (!pluginDescriptor.SupportedVersions.Contains(HrVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
